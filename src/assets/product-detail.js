document.addEventListener('DOMContentLoaded', async function () {
 ...
 const elements = {
    ...
    description: document.getElementById('product-description'), // ✅ thêm mô tả
...
function prepareColorsAndSizes() {
    ...
    function updateUI() {
        ...
        if (availableStock > 0) {
            ...
            elements.addToCartBtn.classList.remove('opacity-50', 'cursor-not-allowed', 'bg-gray-500');
            elements.addToCartBtn.classList.add('bg-black', 'hover:bg-gray-800');
            ...
            elements.addToCartBtn.classList.add('opacity-50', 'cursor-not-allowed', 'bg-gray-500');
            elements.addToCartBtn.classList.remove('bg-black', 'hover:bg-gray-800');
    ...
    try {
        ...
        console.log('[product-detail.js] Product data:', state.product);
        console.log('[product-detail.js] Colors:', state.product.colors);
        console.log('[product-detail.js] Sizes:', state.product.sizes);
...
confirmOkBtn.addEventListener("click", () => {
  ...
  fetch("http://localhost:3001/api/order", {
    ...
    body: JSON.stringify({
      ...
      items: [
        {
          product_id: state.product.id,   // ✅ id số trong DB
          color: state.selectedColor,
          size: state.selectedSize,
          quantity: state.quantity,
          price: state.product.price,
          status: "Đã đặt"                // ✅ trạng thái riêng cho product-detail
        }
      ]
    })
  })
