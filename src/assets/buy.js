...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    // Nếu image là URL đầy đủ thì dùng thẳng, nếu không thì thêm prefix /assets/
    if (state.product.image) {
        elements.image.src = state.product.image.startsWith("http")
            ? state.product.image
            : `/assets/${state.product.image}`;
    } else {
    ...
    function updateUI() {
        if (!state.product) return;
    const availableStock = typeof state.stock === 'number' ? state.stock : 0;
